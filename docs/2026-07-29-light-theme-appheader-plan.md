...
- Accent check: compute the AA ratio of chorus's `--app-accent` on `hsl(0 0% 100%)`; if < 4.5 where used as text, add a light-variant override in the app's root CSS — MUST use the corrected dual-selector pattern (override + media-guarded restore for system+OS-dark; see design §1 correction), never the bare `:root:not([data-theme='dark'])` selector alone.
